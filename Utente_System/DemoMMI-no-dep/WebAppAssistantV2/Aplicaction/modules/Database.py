...
class Database:
    ...
    def insert_one(self, collection, document):
        try:
            ...
            return "Erro ao inserir o docuemnto na base de dados!",77
    ...
    def updateReport(self,collection,query):
        try:
            for id_str, new_views in query.items():
                self.db[collection].update_one({"_id": ObjectId(id_str)}, {"$set": {"views": new_views}} )
            return "Relatório atualizado com sucesso"
        except Exception as e:
            print(f"Error updating report: {e}")
            return "Euro ao atualizar o relatório na base de dados!"
